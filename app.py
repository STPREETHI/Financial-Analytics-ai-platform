            ...
            try:
                ...
                if len(hist) > 0:
                    ...
                    st.download_button(
                        label="Download CSV",
                        data=csv,
                        file_name=f"{data_ticker}_{data_period}_{data_interval}.csv",
                        mime="text/csv",
                    )
                else:
...
# Footer
st.markdown("---")
st.markdown(f"© {datetime.now().year} Financial Analytics AI Platform | Powered by Streamlit, YFinance, and OpenAI")